const dirRoot = '/src/assets/';
const dirIcons = '/assets/icons/';
const dirProjects = '/src/assets/projects/';
const dirServices = '/src/assets/services/';
const dirSocial = '/src/assets/social/';
const dirTechnologies = '/assets/technologies/';

/**
 * An object that contains all the images path from the project
 */
export const rootImages = {
	astronaut: `${dirRoot}div-astronaut.webp`,
	icons: {
		facebook: `${dirIcons}facebook.svg`,
		github: `${dirIcons}github.svg`,
		instagram: `${dirIcons}instagram.svg`,
		english: `${dirIcons}english.svg`,
		spanish: `${dirIcons}spanish.svg`,
	},
	projects: {
		globalredex: `${dirProjects}globalredex-desktop.webp`,
	},
	services: {
		uiux: `${dirServices}uxui.webp`,
		webdev: `${dirServices}webdev.webp`,
		mobile: `${dirServices}mobileapps.webp`,
		marketing: `${dirServices}marketing.webp`,
	},
	technologies: {
		Ai: `${dirTechnologies}Ai.svg`,
		angular: `${dirTechnologies}angular.svg`,
		astro: `${dirTechnologies}astroreal.svg`,
		css: `${dirTechnologies}css.svg`,
		django: `${dirTechnologies}django.svg`,
		express: `${dirTechnologies}express.svg`,
		figma: `${dirTechnologies}figma.svg`,
		flask: `${dirTechnologies}flask.svg`,
		javascript: `${dirTechnologies}javascript.svg`,
		mongodb: `${dirTechnologies}mongo.svg`,
		next: `${dirTechnologies}next.svg`,
		node: `${dirTechnologies}node.svg`,
		postgresql: `${dirTechnologies}postgres.svg`,
		python: `${dirTechnologies}python.svg`,
		react: `${dirTechnologies}react.svg`,
		typescript: `${dirTechnologies}typescript.svg`,
		vue: `${dirTechnologies}vue.svg`,
		Xd: `${dirTechnologies}Xd.svg`,
		crossLogo: `${dirTechnologies}crossfunclogo.svg`,
		techLines: {
			firstLine: `${dirTechnologies}lines/firstline.svg`,
			secondLine: `${dirTechnologies}lines/secondline.svg`,
			thirdLine: `${dirTechnologies}lines/thirdline.svg`,
			fourthLine: `${dirTechnologies}lines/fourthline.svg`,
			fifthLine: `${dirTechnologies}lines/fifthline.svg`,
			sixthLine: `${dirTechnologies}lines/sixthline.svg`,
			seventhLine: `${dirTechnologies}lines/seventhline.svg`,
			eighthline: `${dirTechnologies}lines/eighthline.svg`,
		},
		techIcons: {
			ai: `${dirTechnologies}tech-lines/ai.svg`,
			css: `${dirTechnologies}tech-lines/css.svg`,
			figma: `${dirTechnologies}tech-lines/figma.svg`,
			gmail: `${dirTechnologies}tech-lines/gmail.svg`,
			html: `${dirTechnologies}tech-lines/html.svg`,
			in: `${dirTechnologies}tech-lines/in.svg`,
			js: `${dirTechnologies}tech-lines/js.svg`,
			node: `${dirTechnologies}tech-lines/node.svg`,
			react: `${dirTechnologies}tech-lines/react.svg`,
			twitter: `${dirTechnologies}tech-lines/twitter.svg`,
			xd: `${dirTechnologies}tech-lines/xd.svg`,
		},
	},
	social: {
		whatsapp: `${dirSocial}whatsapplogo.webp`,
		telegram: `${dirSocial}telegramlogo.webp`,
	},
};
